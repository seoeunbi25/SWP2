...
#define _DUTY_MAX 2900 // servo full counterclockwise position (180 degree)
...
void loop() {
  ...
  if (dist_ema < _TARGET_LOW) {
    ...
    float servo = map(dist_ema, _TARGET_LOW, _TARGET_HIGH, _DUTY_MIN, _DUTY_MAX);
    myservo.writeMicroseconds(servo);
    //((_DUTY_MAX - _DUTY_MIN) / 180) * (dist_ema - _TARGET_LOW);
  } else {
